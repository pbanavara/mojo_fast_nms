...
@export
fn PyInit_simple_nms_module() -> PythonObject:
    try:
        var m = PythonModuleBuilder("simple_nms_module")
        m.def_function[simple_nms]("nms", docstring="Run GPU NMS on boxes and scores")
        m.def_function[benchmark_simple_nms]("benchmark", docstring="Benchmark NMS performance")
        return m.finalize()
    except e:
        return abort[PythonObject](String("error creating Python Mojo module:", e))

fn simple_nms(boxes: PythonObject, scores: PythonObject, iou_threshold: PythonObject) raises -> PythonObject:
    """Run GPU NMS on boxes and scores from Python."""
...
fn benchmark_simple_nms(n_boxes: PythonObject, iou_threshold: PythonObject, num_trials: PythonObject) raises -> PythonObject:
